...
- **TTS generating UI**: Player bar shows a dedicated "generating" branch (`isGenerating` state) with `ActivityIndicator`, article info (tappable), and a time estimate label ("Generating audio…" → "~Xs remaining" → "Almost ready…") computed from elapsed time and `chunk.progress` (0.0–1.0 from native layer).
...
│   ├── tts-player-bar.tsx # TTS playback controls bar — uses useTtsContext() directly; shows thumbnail, title, author; tapping left section navigates to article; generating branch with ActivityIndicator + time estimate; returns null when TTS inactive
