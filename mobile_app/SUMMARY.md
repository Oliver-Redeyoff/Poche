...
- **TTS generating UI**: Player bar shows a dedicated "generating" branch (`isGenerating` state) with a circular progress ring, article info (tappable), and a close button. The ring uses a two-semicircle clip technique (no SVG) with solid D-shaped fills and a center hole. Progress is `generationProgress + 0.25` (native 0–1 offset by ¼ for an optimistic head-start), animated with a 1500ms ease so infrequent native callbacks still produce smooth motion. The elapsed-time estimate label was removed in favour of the minimal ring-only UI.
- **TTS background audio**: `setAudioModeAsync` params (`playsInSilentMode: true, shouldPlayInBackground: true`) so audio continues when the app is backgrounded mid-playback.
...
│   ├── tts-player-bar.tsx # TTS playback controls bar — uses useTtsContext() directly; shows thumbnail, title, author; tapping left section navigates to article; generating branch with circular progress ring (no SVG, two-semicircle clip); returns null when TTS inactive
...
├── contexts/
│   └── tts-context.tsx   # Global TTS context (TtsProvider, useTtsContext) — all TTS logic lives here; persists across navigation; two-phase chunked generation; exposes isGenerating/generationProgress; marks article read (progress=100) on natural completion only
