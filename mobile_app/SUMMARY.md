...
- **Reading progress bar**: Thin 3px bar always visible at the bottom of the screen (inside `TtsPlayerBar`); fills to reading progress when TTS is inactive, shows TTS playback progress when active
...
- ✅ **Global TTS context**: All TTS logic lives in `contexts/tts-context.tsx` (no separate hook file); `TtsProvider` wraps root layout so TTS state (`useTtsContext()`) persists across navigation; stores a single `article: Article | null` via `setArticle(article: Article)`; `setContent()` synchronously updates `segmentsRef.current` for race-free immediate playback; `setAudioModeAsync({ playsInSilentModeIOS: true, staysActiveInBackground: true })` for background audio; `UIBackgroundModes: ['audio']` in app.config.js; `close()` saves reading progress to local storage and backend before resetting TTS state
- ✅ **Global TTS player bar**: `TtsPlayerBar` rendered as absolute overlay in `(tabs)/_layout.tsx` above standard tab bar (visible on Home/Library while TTS is playing in background); article screen shows its own inline `TtsPlayerBar` during reading; component uses `useTtsContext()` directly (zero props); shows article thumbnail, title, and author; left section tappable — navigates to article (no-op if already on that article); progress bar animates continuously within each segment using character-count duration estimate at current speed (linear easing via Reanimated); pausing freezes bar via `cancelAnimation`; skipping snaps to correct segment start before re-animating
- ✅ **TTS listen options**: Headphones FAB in article screen opens a native `DropdownMenu` with three options: **Listen from start** (`arrow.counterclockwise`), **Listen from here** (`arrow.forward`, from current scroll position), **Continue listening** (`play.fill`, from stored reading progress — only shown when `0 < readingProgress < 100`); FAB hidden when header is collapsed or TTS is active; "Continue reading" scroll button hidden while TTS is active
