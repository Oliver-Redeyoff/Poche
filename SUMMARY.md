...
- **Neural TTS (Listen)**: On-device text-to-speech via `react-native-sherpa-onnx-offline-tts` (Piper VITS, `vits-piper-en_US-hfc_female-medium-int8`, bundled in app). Headphones FAB in article starts playback from the beginning. Player bar with play/pause and speed (0.75×–2×). iOS only; requires iOS 16.0. Two-phase chunked generation: first 3 segments play within ~1–3s, rest generates in background. "Generating audio…" UI with time estimate during generation. Audio continues when app is backgrounded. Article is marked as read (progress = 100%) only when playback fully completes. Reading progress bar displayed below the article header (scroll-based, 3px).
- **TTS lock screen / Control Center controls**: `expo-media-control` displays the article title, author, and thumbnail (`previewImageUrl`) in the iOS Now Playing card on the lock screen and Control Center; play/pause commands from lock screen/headphones/Control Center are wired back to TTS context; duration estimated from article `wordCount` (~2.5 words/sec); managed entirely in `tts-context.tsx` via `enableMediaControls`, `updateMetadata`, `updatePlaybackState`, `addListener`, and `disableMediaControls`.
...
- ✅ **TTS background audio**: Fixed `setAudioModeAsync` to use correct params (`playsInSilentMode`, `shouldPlayInBackground`) for audio to continue when app is backgrounded; also fixed missing `audio` entry in `UIBackgroundModes` in `ios/Poche/Info.plist` (was declared in `app.json` but never synced to the native file — simulator ignores this but real devices enforce it)
